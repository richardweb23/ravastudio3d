...
export default function VendaActionModal({ sale, mode, locais, vendedores, onClose, onSaved, show }) {
  ...
  const [form, setForm] = useState({ local_id: "", preco_unitario: String(sale.preco_unitario), data: sale.data, vendedor_id: sale.vendedor_id || "", motivo: "" });
  ...
  async function submit(event) {
    ...
    setError("");
    if (!form.motivo.trim()) return setError("Informe o motivo da alteração.");
    const confirmation = returning
      ...
      : "Salvar a edição e atualizar o total desta venda?";
    if (!window.confirm(confirmation)) return;
    submitting.current = true;
    setBusy(true);
    try {
      const common = { p_venda_id: sale.id, p_motivo: form.motivo.trim(), p_versao: sale.versao || 0 };
      const { error: failure } = await supabase.rpc(returning ? "devolver_venda" : "editar_venda", returning
        ? { ...common, p_local_id: form.local_id }
        : { ...common, p_preco_unitario: Number(form.preco_unitario), p_data: form.data, p_vendedor_id: form.vendedor_id || null });
  ...
  return <dialog ref={dialog} className="modal-card registration-dialog" aria-labelledby="sale-action-title" onCancel={event => { event.preventDefault(); if (!busy) onClose(); }}>
    <form className="panel form" onSubmit={submit} aria-busy={busy}>
      <div className="modal-heading"><h2 id="sale-action-title">{returning ? "Retornar venda ao estoque" : "Editar venda"}</h2><button type="button" className="close-modal" aria-label="Fechar" disabled={busy} onClick={onClose}>×</button></div>
      ...
      <fieldset disabled={busy}>
        ...
        <label>Motivo<textarea name="motivo" required maxLength={2000} value={form.motivo} onChange={update} /></label>
      </fieldset>
