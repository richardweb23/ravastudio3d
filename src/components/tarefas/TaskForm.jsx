import { taskAssignees } from "../../lib/tasks.js";
import { useState } from "react";
import { supabase } from "../../supabase.js";

export default function TaskForm({ task, onSaved, onCancel, show, onSavingChange }) {
  const [form, setForm] = useState(() => ({ titulo: task?.titulo || task?.descricao?.slice(0, 150) || "", pessoa_encarregada: task?.pessoa_encarregada || "", descricao: task?.descricao || "", previsao_entrega: task?.previsao_entrega || "" }));
  ...
  async function save(event) {
    ...
    if (saving) return;
    const values = { ...form, titulo: form.titulo.trim(), pessoa_encarregada: form.pessoa_encarregada || null, descricao: form.descricao.trim() };
    if (!values.titulo || !values.descricao || !values.previsao_entrega) { setError("Preencha título, descrição e previsão de entrega."); return; }
    setSaving(true);
    onSavingChange?.(true);
    setError("");
    try {
  ...
  return <form className="panel form task-form" onSubmit={save}>
    ...
    <label>Pessoa encarregada (opcional)<select name="pessoa_encarregada" value={form.pessoa_encarregada} onChange={update}><option value="">Não informado</option>{taskAssignees.map(person => <option key={person} value={person}>{person}</option>)}</select></label>
    {!task?.pessoa_encarregada && task?.responsavel && !form.pessoa_encarregada && <p>Responsável do cadastro antigo: <strong>{task.responsavel}</strong>. Será mantido enquanto você não selecionar uma pessoa.</p>}
